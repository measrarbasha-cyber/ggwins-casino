...
/* ── STICKY SCROLLABLE GAME TABS STRIP ── */
.t-sticky-tabs-wrapper {
  position: sticky;
  top: 60px;
  z-index: 90;
...
.t-scroll-tabs-inner::-webkit-scrollbar {
  height: 5px;
}
.t-scroll-tabs-inner::-webkit-scrollbar-thumb {
  background: #ffd700;
  border-radius: 999px;
}
...
/* ── TOURNAMENTS MAIN VERTICAL FEED ── */
...
@media (max-width: 860px) {
  .t-full-card { grid-template-columns: 1fr; }
}
...
.t-c-icon { font-size: 42px; line-height: 1; }
.t-c-title { font-family: 'Space Grotesk', sans-serif; font-size: 22px; font-weight: 900; color: #fff; }
.t-c-tag { font-size: 11px; font-weight: 800; color: #00e676; text-transform: uppercase; }
...
.t-c-stat-lbl { font-size: 10.5px; color: #94a3b8; font-weight: 700; text-transform: uppercase; }
.t-c-stat-val { font-family: 'Space Grotesk', sans-serif; font-size: 18px; font-weight: 900; color: #ffd700; margin-top: 2px; }

/* Leaderboard */
.t-card-leaderboard-col {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
...
.t-leaderboard-row.rank-2 { background: rgba(192, 192, 192, 0.04); color: #cbd5e1; }
.t-leaderboard-row.rank-3 { background: rgba(205, 127, 50, 0.04); color: #e8976a; }
...
.t-player-name { display: flex; align-items: center; gap: 5px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.btn-enter-t:hover {
  ...
  box-shadow: 0 0 26px rgba(0, 230, 118, 0.65);
}
...
<div class="t-hero">
  ...
  <h1>All Game Tournaments • <span>Scroll &amp; Enter</span></h1>
  <p>Pay <strong>₹50 Registration Entry Fee</strong> (deducted from your Real INR balance) to enter the leaderboard and compete for grand cash prizes!</p>
</div>

<!-- ── STICKY HORIZONTALLY SCROLLABLE GAME TABS BAR ── -->
<div class="t-sticky-tabs-wrapper">
  <div class="t-scroll-tabs-inner" id="sticky-tabs-bar">
    <!-- Generated by JS -->
  </div>
</div>

<!-- ── FULL VERTICAL SCROLLABLE FEED OF ALL 20 GAMES ── -->
<div class="tournaments-feed-container" id="tournaments-feed">
  <!-- Generated by JS -->
</div>
...
function renderAll() {
  ...
  const feed = document.getElementById('tournaments-feed');
  if (!tabsBar || !feed) return;
  ...
  // 1. Render Sticky Tabs
  tabsBar.innerHTML = ALL_GAMES.map((g, idx) => `
    <button class="t-tab-btn ${idx === 0 ? 'active' : ''}" id="tab-btn-${g.id}" onclick="scrollToGame('${g.id}')">
      <span>${g.icon}</span> ${g.game}
    </button>
  `).join('');

  // 2. Render Full Vertical Feed of All 20 Games
  feed.innerHTML = ALL_GAMES.map(g => {
    const registered = isRegistered(g.id);
    const uScore = getUserScore(g.id);

    return `
      <div class="t-full-card" id="card-${g.id}" onclick="openRegistrationModal('${g.id}')" style="cursor:pointer">
        <div class="t-card-info-col">
          <div>
            <div class="t-card-header-row">
              <div class="t-c-icon">${g.icon}</div>
              <div>
                <div class="t-c-title">${g.game}</div>
                <div class="t-c-tag">🏆 ${g.tag} Tournament</div>
              </div>
            </div>

            <p style="font-size:13.5px;color:#cbd5e1;line-height:1.5;margin:14px 0">
              Compete against top champions in <strong>${g.game}</strong>. Entry fee is strictly <strong>₹50.00</strong> deducted from your Real INR Balance.
            </p>

            <div class="t-c-stats-grid">
              <div class="t-c-stat">
                <div class="t-c-stat-lbl">Grand Prize Pool</div>
                <div class="t-c-stat-val">₹${g.prize.toLocaleString('en-IN')}</div>
              </div>
              <div class="t-c-stat">
                <div class="t-c-stat-lbl">Entry Fee</div>
                <div class="t-c-stat-val" style="color:#00e676">₹${g.fee}.00</div>
              </div>
              <div class="t-c-stat">
                <div class="t-c-stat-lbl">Enrolled Players</div>
                <div class="t-c-stat-val" style="font-size:16px;color:#cbd5e1">👥 ${g.enrolled + (registered ? 1 : 0)} Players</div>
              </div>
              <div class="t-c-stat">
                <div class="t-c-stat-lbl">1st Place Share</div>
                <div class="t-c-stat-val" style="font-size:16px;color:#ffd700">🥇 60% (₹${Math.floor(g.prize * 0.6).toLocaleString('en-IN')})</div>
              </div>
            </div>
          </div>
          ...
          <div>
            ${registered ? `
              <button class="btn-enrolled-t" onclick="event.stopPropagation(); window.location.href='${g.url}'">
                ✅ Enrolled &amp; Active • Launch ${g.game} &amp; Climb 🎮
              </button>
            ` : `
              <button class="btn-enter-t" onclick="event.stopPropagation(); openRegistrationModal('${g.id}')">
                ⚡ Pay ₹50 Entry Fee &amp; Enter ${g.game}
              </button>
            `}
          </div>
        </div>

        <div class="t-card-leaderboard-col">
...
print("SUCCESS: Fully scrollable tournaments page written with sticky tabs and full 20-game feed!")