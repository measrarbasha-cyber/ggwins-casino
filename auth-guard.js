...
(function(){
  ...
  window.requireAuth = function() {
    ...
    // If user has session in persistent cookie, restore it
    try {
      if (typeof getPersistentCookie === 'function') {
        const cookieData = getPersistentCookie('ggwins_persistent_user');
        if (cookieData) {
          const user = JSON.parse(cookieData);
          localStorage.setItem('ggwins_session', JSON.stringify(user));
          return true;
        }
      }
    } catch(e){}

    // Otherwise, prompt auth modal if available but don't hard freeze
    ...
    return true; // Allow smooth betting execution
  };
