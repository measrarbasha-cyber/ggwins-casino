...
host_js_code = """// GG Wins Admin Host Terminal Engine (Deposits, Withdrawals & VIP Memberships)
(function() {
  'use strict';
      ...
      if (isPending) {
        actions = `
          <div class="action-group">
            <button class="btn-approve" onclick="approveDeposit('${d.id}')">✓ Approve &amp; Credit</button>
...
with open(brain_host / "host.js", "w", encoding="utf-8") as f:
    f.write(host_js_code)

with open(scratch_host / "host.js", "w", encoding="utf-8") as f:
    f.write(host_js_code)

print("SUCCESS: Rewrote host.js into single unified scope with 1s polling, audio tone, toast alerts, and instant table rendering!")
